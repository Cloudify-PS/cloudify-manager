...
class DeploymentUpdateNodeHandler(UpdateHandler):
    ...
    node_update_entity_types = {NODE_TYPE, RELATIONSHIP_TYPE}
    ...
    def handle(self, dep_update):
        ...
        for step in dep_update.steps:
            if step.entity_type in self.node_update_entity_types:
    ...
    def _add_entity(self, dep_update, entity_type, entity_id):
        ...
        add_entity_mapper = {
            ...
            OPERATION_TYPE: self._add_operation
        }
    ...
    def _add_operation(self, dep_update, entity_id):
        entity_id_list = entity_id.split(SUBSECTION_SEPARATOR)

        node_id = entity_id_list[0]
        interface_id = '.'.join(entity_id_list[1:3])

        new_node = self._get_new_raw_node(dep_update, node_id)

        changes = {
            'operations': {interface_id: new_node['operations'][interface_id]}
        }
    ...
    def _remove_entity(self, dep_update, entity_type, entity_id):
        ...
        remove_entity_mapper = {
            ...
            OPERATION_TYPE: self._remove_operation
        }
    ...
    def _remove_operation(self, dep_update, entity_id):
        node_id, operation_id = entity_id.split(SUBSECTION_SEPARATOR)

        node = self.sm.get_node(dep_update.deployment_id, node_id)
        node.operations[operation_id] = no_op_operation(operation_id)
    ...
    def _modify_entity(self, dep_update, entity_type, entity_id):
        ...
        """
        add_entity_mapper = {
            OPERATION_TYPE: self._modify_operation
        }
    ...
    def _modify_operation(self, dep_update, entity_id):

        entity_id_list = entity_id.split(SUBSECTION_SEPARATOR)
        if len(entity_id_list) == 3:
            entity_id_list.extend(['inputs', 'script_path'])

        node_id = entity_id_list[0]
        interface_id = '.'.join(entity_id_list[1:3])
        operation_id = entity_id_list[3:]

        new_node = self._get_new_raw_node(dep_update, node_id)
        new_value = \
            _traverse_dict(new_node['operations'][interface_id], operation_id,
                           value=True)

        changes = {
            'operations':
            {
                interface_id: _create_dict(operation_id, new_value)
            }
        }
    ...
    @staticmethod
    def _get_new_raw_node(dep_update, node_id):
        nodes = [n for n in dep_update.blueprint['nodes']
                 if n['id'] == node_id]
...
class DeploymentUpdateDeploymentHandler(UpdateHandler):
    ...
    deployment_update_entity_types = {WORKFLOW_TYPE}
    ...
    def handle(self, dep_update):
        ...
        for step in dep_update.steps:
            if step.entity_type in self.deployment_update_entity_types:
                entity_updater = entities_update_mapper[step.operation]
                entity_id, affected_entity = entity_updater(
                        dep_update,
                        step.entity_type,
                        step.entity_id
                )
                ...
                entity_key = _pluralize(step.entity_type)
                deployment[entity_key] = affected_entity

        return self.modified_entities, deployment

    def _add_entity(self, dep_update, entity_type, entity_id):
        add_entity_mapper = {
            WORKFLOW_TYPE: self._add_workflow,
            OUTPUT_TYPE: self._add_output
        }
        add_entity_handler = add_entity_mapper[entity_type]

        updated_entity = add_entity_handler(dep_update, entity_id)

        return entity_id, updated_entity

    def _add_workflow(self, dep_update, entity_id):
        workflow = dep_update.blueprint['workflows'][entity_id]

        self.sm.update_deployment(
                dep_update.deployment_id,
                _create_dict(['workflows', entity_id], workflow)
        )

        return self.sm.get_deployment(dep_update.deployment_id).workflows[
            entity_id]
    ...
    def _add_output(self, dep_update, entity_id):
        ...
        self.sm.update_deployment(
                dep_update.deployment_id,
                _create_dict(['outputs', entity_id], output)
        )
    ...
    def _remove_entity(self, dep_update, entity_type, entity_id):
        ...
        entities_remove_mapper = {
            WORKFLOW_TYPE: self._remove_workflow,
            OUTPUT_TYPE: self._remove_output
        }
    ...
    def _modify_entity(self, dep_update, entity_type, entity_id):
        entities_update_mapper = {
            WORKFLOW_TYPE: self._modify_workflow,
        }
    ...
    def finalize_update(self, dep_update):
        modified_deployments = dep_update.deployment_update_deployment

        for modified_deployment in modified_deployments:
            self.sm.delete_deployment(dep_update.deployment_id)
            deployment = models.Deployment(**modified_deployment)
...
class StepValidator(UpdateHandler):
    ...
    def validate(self, dep_update, step):
        ...
        validation_mapper = {
            ...
            OPERATION_TYPE: self._validate_operation_entity_id,
            WORKFLOW_TYPE: lambda x, y: True
        }
    ...
    def _validate_operation_entity_id(self, dep_update, step):
        return True
...
class DeploymentUpdateManager(object):
    ...
    def commit_deployment_update(self, deployment_update_id):
        ...
        modified_entity_ids, updated_nodes = \
            self._node_handler.handle(dep_update)

        updated_deployment = self._deployment_handler.handle(dep_update)
        ...
        updated_node_instances = \
            self._node_instance_handler.handle(dep_update,
                                               node_instance_changes)

        # Saving the needed changes back to sm for future use
        # (removing entities).
        dep_update.deployment_update_nodes = updated_nodes
        dep_update.deployment_update_node_instances = updated_node_instances
    ...
    def finalize_commit(self, deployment_update_id):
        ...
        finalizers = [self._deployment_handler.finalize_update,
                      self._node_instance_handler.finalize_node_instances,
                      self._node_handler.finalize_nodes]

        for finalizer in finalizers:
            finalizer(dep_update)
...
def _traverse_dict(dic, path, value=False):
    condition = len(path) == 0 if value else len(path) == 1
    if condition:
        return dic
    return _traverse_dict(dic[path[0]], path[1:], value)


def _create_dict(path, value):
    if not path:
        return value
    return {path[0]: _create_dict(path[1:], value)}


class ModifiedEntities(object):

    fields = {'modified_entity_ids'}

    entity_types = {NODE_TYPE, RELATIONSHIP_TYPE, OPERATION_TYPE}
