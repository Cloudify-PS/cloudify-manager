    ...
    def _prepare_and_submit_blueprint(cls, file_server_root,
        ...
        try:
            ...
            update = get_deployment_updates_manager().stage_deployment_update(
                    deployment_id,
                    os.path.join(file_server_root, app_dir, app_file_name),
                )

            # moving the app directory in the file server to be under a
            # directory named after the deployment id
            shutil.move(
                    os.path.join(file_server_root, app_dir),
                    os.path.join(
                            file_server_root,
...
class DeploymentUpdates(SecuredResource):
    ...
    def post(self, **kwargs):
        ...
        blueprint_app_name = \
            query_params.get('application_file_name',
                             CONVENTION_APPLICATION_BLUEPRINT_FILE)
